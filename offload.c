...
// calls MPI directly. Includes fixes for request initialization and continuous
// MPI progress even when the command queue is empty.
...
volatile int done_flag[MAX_REQ];      // volatile so other thread can see updates
...
void init_request_pool(void) {
    ...
    for (int i = 0; i < MAX_REQ; i++) {
        free_list[i]  = i + 1;
        done_flag[i]  = 0;
        req_pool[i]   = MPI_REQUEST_NULL;   // IMPORTANT: start as valid null requests
    }
...
void free_req_index(int idx) {
    pthread_mutex_lock(&free_lock);
    req_pool[idx] = MPI_REQUEST_NULL;  // not strictly required, but clean
...
// blocking enqueue (used by compute thread)
...
// NON-BLOCKING dequeue (used by offload thread)
    ...
    while (!shutdown_flag) {
        ...
        if (got) {
            if (cmd.type == CMD_SHUTDOWN) {
                // main thread is telling us to exit
                shutdown_flag = 1;
                continue;
            }
        ...
        // ALWAYS drive MPI progress a bit, even if no new commands arrived.
        int index, flag;
        MPI_Status st;
        ...
        // Optional: insert a tiny yield or sleep here if CPU usage is too high.
    }

    return NULL;
}

// ---------- wrapper API for compute thread ----------

static MPI_Request make_fake(int idx) {
    return (MPI_Request)(long)(idx + 1);
}

static int get_index(MPI_Request r) {
    return (int)((long)r) - 1;
}
...
int my_Wait(MPI_Request *req) {
    ...
    // Busy-wait until offload thread marks this request done.
    while (!done_flag[idx]) {
        // Could call sched_yield() here if you like.
    }
...
static void do_compute(double *a, int n) {
    // Same stencil as in baseline/comm_self
...
void *compute_thread_main(void *arg) {
    ...
    for (int step = 0; step < NSTEPS; step++) {
        ...
        my_Irecv(&a[0],         HALO_SIZE, MPI_DOUBLE, left,  0, MPI_COMM_WORLD, &r[0]);
        my_Irecv(&a[N_LOCAL+1], HALO_SIZE, MPI_DOUBLE, right, 1, MPI_COMM_WORLD, &r[1]);
        my_Isend(&a[1],         HALO_SIZE, MPI_DOUBLE, left,  1, MPI_COMM_WORLD, &r[2]);
        my_Isend(&a[N_LOCAL],   HALO_SIZE, MPI_DOUBLE, right, 0, MPI_COMM_WORLD, &r[3]);
...
int main(int argc, char **argv) {
    ...
    // Tell offload thread to shut down
    Command c;
    c.type = CMD_SHUTDOWN;
    enqueue_cmd(&c);
    // shutdown_flag will be set inside thread when it sees CMD_SHUTDOWN
